...
class PlcEdgeTests(unittest.TestCase):
    ...
    def test_live_ui_exposes_plc_signal_state(self) -> None:
        ...
        self.assertNotIn('id="plc-measurement-frame"', LIVE_TEMPLATE)
        self.assertNotIn("/api/live/measurement.jpg?event_key=", LIVE_SCRIPT)
        self.assertNotIn("exactFrameReady", LIVE_SCRIPT)
        self.assertIn("presentationDelayMs", LIVE_SCRIPT)
        ...
        self.assertNotIn('addEventListener("stalled", reconnectLiveVideo)', LIVE_SCRIPT)
        ...
        self.assertIn("livePresentationDelayMs", LIVE_SCRIPT)
        self.assertIn("trimLiveBuffer", LIVE_SCRIPT)
        self.assertIn("/api/live/frame?metadata=1", LIVE_SCRIPT)
        self.assertNotIn('src="/api/live/stream.mp4"', LIVE_TEMPLATE)
        self.assertIn("/api/live/stream.mp4?generation=", LIVE_SCRIPT)
        self.assertIn("client_id=", LIVE_SCRIPT)
